"""
Script 8: NPT equilibration at 300K
Reads: 7_{filename}
Outputs: 8_{filename}
"""
...
# Find input structure (7_filename)
xyz_files = glob.glob("7_*.xyz")
if not xyz_files:
    raise FileNotFoundError("No 7_*.xyz file found!")
input_file = xyz_files[0]
base_name = os.path.splitext(os.path.basename(input_file))[0][2:]  # Remove "7_" prefix
...
traj = Trajectory(f"lowtemp_equilibration_{base_name}.traj", "w", atoms)
...
# Save output
output_file = f"8_{base_name}.xyz"
