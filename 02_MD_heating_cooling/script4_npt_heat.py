"""
Script 4: NPT heating from 300K to 1000K
Reads: final_{filename}
Outputs: 4_{filename}
"""
...
# Find input structure (final_filename)
xyz_files = glob.glob("final_*.xyz")
if not xyz_files:
    raise FileNotFoundError("No final_*.xyz file found!")
input_file = xyz_files[0]
base_name = os.path.splitext(os.path.basename(input_file))[0][6:]  # Remove "final_" prefix
...
# NPT heating parameters
start_temp = 300.0  # K
end_temp = 1000.0   # K
heating_time_ps = 20.0  # ps
timestep_fs = 1.0
...
total_steps = int(heating_time_ps * 1000 / timestep_fs)  # ps to fs to steps
heating_rate = (end_temp - start_temp) / heating_time_ps  # K/ps

print(f"Heating from {start_temp} K to {end_temp} K over {heating_time_ps} ps")
print(f"Heating rate: {heating_rate:.1f} K/ps")
print(f"Total steps: {total_steps}")

MaxwellBoltzmannDistribution(atoms, temperature_K=start_temp)
Stationary(atoms)

# Setup NPT with heating
...
traj = Trajectory(f"heating_{base_name}.traj", "w", atoms)
...
def update_temperature():
    ...
    target_temp = start_temp + heating_rate * current_time_ps
...
# Save output
output_file = f"4_{base_name}.xyz"
write(output_file, atoms)
print(f"Heating completed. Final temperature: {atoms.get_temperature():.1f} K")
