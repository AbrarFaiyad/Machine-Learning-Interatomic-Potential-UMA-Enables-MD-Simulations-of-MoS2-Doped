"""
Script 6: NVT equilibration at 1000K for 100ps
Reads: 5_{filename}
Outputs: 6_{filename}
"""
...
# Find input structure (5_filename)
xyz_files = glob.glob("5_*.xyz")
if not xyz_files:
    raise FileNotFoundError("No 5_*.xyz file found!")
input_file = xyz_files[0]
base_name = os.path.splitext(os.path.basename(input_file))[0][2:]  # Remove "5_" prefix
...
traj = Trajectory(f"nvt_equilibration_{base_name}.traj", "w", atoms)
...
def log_progress():
    ...
    with open("nvt_density_log.txt", "a") as f:
...
# Run equilibration
print("Starting NVT equilibration at 1000K...")
dyn.run(total_steps)
...
# Save output
output_file = f"6_{base_name}.xyz"
